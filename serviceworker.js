  ...
  chrome.storage.sync.get(
    ...
    function (get) {
      ...
      if (nameCheck === true && code.themeName.length < 26) {
        ...
        let index = vt.findIndex(
          (x) => x.themeName.toLowerCase() === code.themeName.toLowerCase()
        );
        if (index !== -1) {
  ...
  chrome.storage.sync.get(
    ...
    function (get) {
      ...
      if (sc.activated === true) {
        ...
        if (index === -1) {
          var diff =
            (86400 -
              ...
              Number(sc.schedule[0].time.substring(0, 2)) * 3600 +
              Number(sc.schedule[0].time.substring(3, 5)) * 60) *
            1000;
        } else {
          var diff =
            (Number(sc.schedule[index].time.substring(0, 2)) * 3600 +
              Number(sc.schedule[index].time.substring(3, 5)) * 60 -
              (hours * 3600 + minutes * 60)) *
            1000;
        }
        if (index === -1 || index === 0) {
        ...
        chrome.alarms.create("themeChange", {
          when: Math.floor(epoch / 60000) * 60000 + diff,
        });
      }
    }
  );
...
function normaliseFormattingToolbarOrders() {
  chrome.storage.sync.get(
    ...
    (settings) => {
      ...
      const newButtons = possibleButtons.filter(
        (x) => existingButtons.indexOf(x) === -1
      );
...
chrome.runtime.onInstalled.addListener(function (details) {
  ...
  if (details.reason === "install" || details.previousVersion * 1 < 3)
    defaultSetup();
});
